...
def test_reconcile_results(test_parameters, results_datamodel):
    """Verify a result file passed to reconcile results"""
    RC._results_datamodel = None
    RC._reconcile_results(test_parameters)

    assert results_datamodel == RC._results_datamodel


def test_compile_yaml(test_parameters, results_datamodel):
    fname = "reports/results/results-testdata.yml"
    dname = "reports/results"

    with open(fname, "w", encoding="utf-8") as yaml_file:
        yaml.dump(test_parameters, yaml_file)

    RC._compile_yaml_data(dname)
